...
from models.simple_mnsit import SimpleNet
...
class VGG(nn.Module):
    ...
    def get_feature(self, x):
        # conv(features) 직후에서 임베딩 추출 → ResNet(avg_pool 직후)과 동등 위치.
        # (VGG conv 출력은 CIFAR 32x32에서 이미 512x1x1 = flatten 512차원, ResNet과 일치)
        # 기존엔 classifier[:6](FC 2층 통과)에서 뽑아 위치가 달랐음 → 공정비교 위해 FC 제거.
        x = self.features(x)
        x = x.view(x.size(0), -1)
        return x
