...
def layer_analysis_no_acc(model_param, args, mal_train_dataset, mal_val_dataset, threshold=0.8):
    if args.model == 'resnet':
        _ncls = 100 if getattr(args, 'dataset', '') == 'cifar100' else 10
        model = ResNet18(num_classes=_ncls).to(args.device)
    elif args.model == 'VGG':
    ...
    _, back_acc = test(model_malicious, mal_val_dataset, args)  # 풀 악성모델 BSR(기준) — no_acc 버전 누락 버그 수정
...
def get_key_value_bsr(model_param, args, mal_train_dataset, mal_val_dataset):
    if args.model == 'resnet' and args.dataset in ('cifar', 'mnist', 'fmnist'):
        model = ResNet18(num_classes=10).to(args.device)   # mnist/fmnist는 Grayscale(3)로 3채널
    elif args.model == 'VGG':
        _nc = 100 if getattr(args, 'dataset', '') == 'cifar100' else 10
        model = vgg19_bn(num_classes=_nc).to(args.device)
        ...
        model = ResNet18(num_classes=100).to(args.device)
