...
from defense.flame import flame_analysis
...
from client.MaliciousUpdate import LocalMaliciousUpdate
...
def get_key_value_bsr(model_param, args, mal_train_dataset, mal_val_dataset):
    if args.model == 'resnet':
        _nc = 100 if getattr(args, 'dataset', '') == 'cifar100' else 10
        model = ResNet18(num_classes=_nc).to(args.device)
    elif args.model == 'VGG':
        _nc = 100 if getattr(args, 'dataset', '') == 'cifar100' else 10
        model = vgg19_bn(num_classes=_nc).to(args.device)
...
def attacker(list_mal_client, num_mal, attack_type, dataset_train, dataset_test, dict_users, net_glob, args, idx=None,prev_update=None,edge=None,dba_pos=None):
    num_mal_temp=0
    if args.ada_mode == 20:
    ...
    if attack_type == "layerattack_ER_his" or attack_type == "LFA" or attack_type == "LPA":
        ...
        w, loss = local.train(
            net=copy.deepcopy(net_glob).to(args.device), test_img=test_img, args=args)
    elif attack_type == 'neu':
        w, loss = local.train(
            net=copy.deepcopy(net_glob).to(args.device), test_img=test_img, args=args)
    else:
        w, loss = local.train(
