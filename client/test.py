...
def test_img(net_g, datatest, args, test_backdoor=False):
    ...
    if args.attack == 'edge':
        acc, loss, bsr = get_edge(net_g, datatest, args)
        return acc, loss, bsr
    args.watermark = None
    args.apple = None
    net_g.eval()
    ...
    back_correct = 0
    back_num = 0
    for idx, (data_, target_) in enumerate(data_loader):
...
def get_edge(net_g,vanilla_loader, targetted_loader,args):
    net_g.eval()
    # testing
    test_loss = 0
    correct = 0
    # vanilla_test, vanilla_loader = load_saved_dataset('vanilla_test.pkl')
    # targetted_test, targetted_loader = load_saved_dataset('targetted_test.pkl')
    # # data_loader = DataLoader(datatest, batch_size=args.bs)
    back_correct = 0
    back_num = 0
    for idx, (data_, target_) in enumerate(vanilla_loader):
        data = copy.deepcopy(data_)
        target = copy.deepcopy(target_)
        if args.gpu != -1:
            data, target = data.to(args.device), target.to(args.device)
    ...
    test_loss /= len(vanilla_loader.dataset)
    accuracy = 100.00 * correct / len(vanilla_loader.dataset)
    for idx, (data_, target_) in enumerate(targetted_loader):
        data = copy.deepcopy(data_)
        target = copy.deepcopy(target_)
        if args.gpu != -1:
            data, target = data.to(args.device), target.to(args.device)
    ...
    test_loss /= len(targetted_loader.dataset)
    back_accu = 100.00 * back_correct / len(targetted_loader.dataset)
